...
int Protocol::WriteGPT(TCHAR *szPartName, TCHAR *szBinFile)
{
  ...
  if (LoadPartitionInfo(szPartName, &partEntry) == ERROR_SUCCESS){
    ...
    swprintf_s(cmd_pkt, MAX_XML_LEN, L"<program SECTOR_SIZE_IN_unsigned charS=\"%i\" num_partition_sectors=\"%i\" physical_partition_number=\"0\" start_sector=\"%i\"/", DISK_SECTOR_SIZE, (int)partEntry.num_sectors, (int)partEntry.start_sector);
...
int Protocol::WipeDiskContents(uint64 start_sector, uint64 num_sectors, TCHAR *szPartName)
{
  ...
  swprintf_s(cmd_pkt, MAX_XML_LEN, L"<program SECTOR_SIZE_IN_unsigned charS=\"%i\" num_partition_sectors=\"%i\" physical_partition_number=\"0\" start_sector=\"%i\"/", DISK_SECTOR_SIZE, (int)num_sectors, (int)start_sector);
  Partition partition;
